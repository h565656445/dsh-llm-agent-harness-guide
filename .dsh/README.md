...
将 `.dsh` 目录复制到 `$DSH_HOME/.agent-presets/hermes-llm-agent-harness-guide/`：
...
重启 DeepSeek Harness 后，在会话预设列表选择 `LLM Agent 控制平面设计专家`。

### 方式二：仅安装技能（skill）

将 SKILL.md 复制到 `$DSH_HOME/skills/dsh-llm-agent-harness-guide/`：

```powershell
$dst = Join-Path $env:DSH_HOME "skills\dsh-llm-agent-harness-guide"
New-Item -ItemType Directory -Force -Path $dst | Out-Null
Copy-Item ".\skills\dsh-llm-agent-harness-guide\SKILL.md" $dst -Force
```
